...
const navItems = [
  { title: "Օրհներգեր 1-800", route: "" },
  { title: "Այբբենական ցուցիչ", route: "alphabetical" },
  { title: "Բովանդակություն", route: "content" },
  { title: "Պատմություն", route: "history" },
  { title: "Էջանիշեր", route: "bookmark" },
  { title: "Նախաբան", route: "preface" },
  { title: "Տեղեկություն", route: "reference" },
  { title: "Ծրագրի մասին", route: "about" },
  { title: "Կարգավորումներ", route: "settings" },
];
  ...
  useEffect(() => {
    if (currentNumber.length && pathname === `/hymns/${currentNumber}`) {
      ...
      if (currentNumber.length > 1) {
        setTitle(`Օրհներգեր ${currentNumber.slice(0, 3).map((number) => " " + number)}${currentNumber.length > 3 ? " ..." : ""}`);
      } else {
        setTitle(`Օրհներգ ${currentNumber}<sup>${currentHymn?.sign}</sup>`);
      }
